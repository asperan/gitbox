...
pub struct LicenseSubCommand {
    #[arg(short, long, default_value = "LICENSE", help = "Set the license file name")]
    filename: String,
}
...
impl LicenseSubCommand {
    pub fn create_license(&self) {
        ...
        match license_text {
            ...
            Err(e) => print_error_and_exit(&format!("Failed to downlad license text: {}", &e.to_string())),
        }
    }
    ...
    fn ask_for_license_url(&self) -> String {
        ...
        match list {
            Ok(l) => {
                ...
                let answer = prompt_one(Question::raw_select("license")
            ...
            Err(e) => print_error_and_exit(&format!("Failed to get license list: {}", &e.to_string())),
        }
    }
    ...
    fn download_license(&self, url_path: &str) -> Result<String, ureq::Error> {
        ...
        match raw_license_page {
            Ok(page) => {
                let license_text_regex = RegexBuilder::new(&format!("{}(.*){}", HTML_LICENSE_PREFIX, HTML_LICENSE_SUFFIX))
