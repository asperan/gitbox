...
impl MetadataGenerator {
    pub fn generate<'a>(specs: impl IntoIterator<Item = &'a MetadataSpecs>) -> Option<String> {
        let mut unique_specs: Vec<MetadataSpecs> =
            Vec::with_capacity(MetadataSpecs::value_variants().len());
        specs.into_iter().for_each(|s| {
            if !unique_specs.contains(s) {
                unique_specs.push(*s);
            }
        });
        if unique_specs.is_empty() {
            ...
            unique_specs
                .iter()
                .map(MetadataGenerator::spec_to_string)
        ...
        match spec {
            ...
            MetadataSpecs::Sha => {
                ...
                if last_commit_sha.status.success() {
                    ...
                    print_cli_error_message_and_exit(
                        &last_commit_sha.stderr,
